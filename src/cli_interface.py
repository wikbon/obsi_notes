...
from src.utils.helpers import generate_frontmatter, should_skip_file
...
class NoteCLI:
    ...
    def __init__(self, vault_path: Optional[str] = None, config_path: Optional[str] = None):
        ...
        if selected_impl == "llama-cpp-python":
            ...
            questions = [
                inquirer.List(
                    "model", message="Select the model to use:", choices=list(MODELS.keys())
                )
            ]
        ...
        self.extractor = AtomicNoteExtractor(
            vault_path=vault_path, config_path=config_path, verbose=True, llm_handler=self.llm
        )
    ...
    def get_daily_notes(self) -> List[Path]:
        ...
        daily_notes = [
            f
            for f in all_files
            if f.stem.replace(".md", "").replace("-", "").isdigit() and len(f.stem) == 10
        ]
    ...
    def select_note(self) -> Optional[Path]:
        ...
        choices = [
            f"{note.stem} - {note.relative_to(self.parser.vault_path)}" for note in daily_notes
        ]

        questions = [
            inquirer.List("note", message="Select a daily note", choices=choices, carousel=True)
        ]
    ...
    def chat_about_note(self, note_path: Path) -> None:
        ...
        initial_response = self.llm.create_chat_completion(
            messages=self.llm.get_history(), temperature=0.7
        )
    ...
    def process_atomic_notes(self, note_path: Path) -> None:
        ...
        try:
            ...
            atomic_notes = self.extractor.process_file(
                file_path=str(note_path), export_json=export_json, temperature=0.7
            )
